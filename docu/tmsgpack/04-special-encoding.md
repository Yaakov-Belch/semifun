[[tmsgpack:special-encoding]]
# Special encoding: custom serialization for non-dataclass types

* How to define custom encode/decode logic for types that are not dataclasses.
* Subclass `TmsgpackSpecial` and define one encode/decode pair: dict, list, or bytes mode.
* `Inject[T]` works on both encode and decode signatures.


## Basics

Register a `TmsgpackSpecial` subclass with the `#::` comment.  The key
...
```python
from tmsgpack.codec_special import TmsgpackSpecial
...
`TmsgpackSpecial.__init_subclass__` validates at class definition time:
...
The `isinstance(..., TmsgpackSpecial)` check in `codec_spec_to_codec_handler`
runs **before** `is_dataclass`, so a `@dataclass` decorator on a
`TmsgpackSpecial` subclass is harmless — the special path takes priority.
